...
begin
	...
	Pkg.add([
			...
			# Pkg.PackageSpec(name="Distributions", version="0.24.14")
			])
...
example_vector = [0.0, 0.4, 0.3, 0.2, 0.1, 0.0, 0.7, 0.0, 0.7, 0.9]
...
function get_reds(image::AbstractMatrix)
	image_red_values = get_red.(image)
	return image_red_values
end
...
philip_reds = value_as_color.(get_reds(philip_head))
...
# ╔═╡ 4f74b7a0-75e5-11eb-2b7b-2f31dcda1860
function get_greens(image::AbstractMatrix)
	image_green_values = get_green.(image)
	return image_green_values
end

# ╔═╡ 938f9982-75e5-11eb-3362-8df126d4dc79
function get_blue(pixel::AbstractRGB)
	return pixel.b
end

# ╔═╡ a608a428-75e5-11eb-028a-bf9c71498799
function get_blues(image::AbstractMatrix)
	image_blue_values = get_blue.(image)
	return image_blue_values
end
...
function mean_color(image)
	...
	av_red = mean(reds)
	av_green = mean(greens)
	av_blue = mean(blues)
	return RGB(av_red, av_green, av_blue)
end
...
function invert(color::AbstractRGB)
	inverted_color = RGB(1-color.r, 1-color.g, 1-color.b)
...
function quantize(color::AbstractRGB)
	new_color = RGB(quantize(color.r), quantize(color.g), quantize(color.b))
	return new_color
end
...
function quantize(image::AbstractMatrix)
	quantized_philip = quantize.(image)
	return quantized_philip
end

# ╔═╡ f6a655f8-ee07-11ea-13b6-43ca404ddfc7
quantize(0.0267), quantize(0.99)
...
# ╔═╡ ede287a0-75f4-11eb-19ab-5debeeaeb62a
s = 0.9

# ╔═╡ e7ad979e-75f4-11eb-2f88-fd588143eb4d
begin
	Pkg.add("Distributions")
	using Distributions
	intensity = rand(-s:.0001:s, 12)
end

# ╔═╡ f38b198d-39cf-456f-a841-1ba08f206010
function noisify(x::Number, s)
	intensity = rand(-s:.0001:s)
	new_x = x+intensity
	new_x = clamp(new_x, 0, 1)
	return new_x
end
...
function noisify(image::AbstractMatrix, s)
	noisy_image = noisify.(image, s)
	return noisy_image
end

# ╔═╡ 1ea53f41-b791-40e2-a0f8-04e13d856829
noisify(0.5, 0.0) # edit this test case!
...
The image is unrecognisable with intensity 2 because as the window for random noise intensity increases the expected value of noise intensity increases as well.
"""
...
function custom_filter(pixel::AbstractRGB)
	new_pixel = RGB(pixel.b, mean([pixel.g, pixel.b, pixel.r]), pixel.r)
	return new_pixel
end
