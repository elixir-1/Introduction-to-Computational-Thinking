...
# ╔═╡ 2a089f50-8629-11eb-0862-53e2549c6049
begin
	String(rand(alphabet, 30))
	Text("jellyfish")
end
...
# ╔═╡ d63382fa-862d-11eb-0a61-8bea6b863b2f
charDigits = '0':'9'

# ╔═╡ e2a27762-862d-11eb-142e-95c33b5bc687
'4' in charDigits

# ╔═╡ 12794dde-862f-11eb-1c3a-8d98a8f81a8d
6÷5 - 6/5

# ╔═╡ d60a4df0-862e-11eb-1e37-d97dbd551a64
...
filter(isMultipleOf5, [6, 7, 8, 9, -5, 15, 225])
...
# ╔═╡ a6909d62-8634-11eb-08d0-27d46bd69eb8
cleaned_sentence_2_1 = filter(isinalphabet, map(s->lowercase(s), messy_sentence_2))
...
function clean(text)
	...
	unaccented_text = unaccent(text)
	lowercase_text = map(s->lowercase(s), unaccented_text)
	clean_text = filter(isinalphabet, lowercase_text)
	return clean_text
end
...
unused_letters = [c for c ∈ alphabet if sample_freqs[index_of_letter(c)] == 0.0]
...
# ╔═╡ 0e72a876-8651-11eb-2fe7-cf38fc780993
string('v', 'v')
...
# ╔═╡ 24df8e66-8652-11eb-25d3-d33c2918f09c
['t'-'a'+1, 'h'-'a'+1]

# ╔═╡ 74d23eb4-8652-11eb-1f17-0f8ed45fee99
function getIndexFromLetter(c)
	findfirst(isequal(c), alphabet)
end

# ╔═╡ 1b4c0c28-f9ab-11ea-03a6-69f69f7f90ed
th_frequency = sample_freq_matrix[getIndexFromLetter('t'), getIndexFromLetter('h')]

# ╔═╡ 1f94e0a2-f9ab-11ea-1347-7dd906ebb09d
md"""👉 What about `"ht"`?"""

# ╔═╡ 41b2df7c-f931-11ea-112e-ede3b16f357a
ht_frequency = sample_freq_matrix[getIndexFromLetter('h'), getIndexFromLetter('t')]
...
# ╔═╡ 225f079c-8653-11eb-055f-53ebc617314b
begin
	mat = fill(0.0, (27,27))
	for x in alphabet, y in alphabet
		mat[getIndexFromLetter(x), getIndexFromLetter(y)] = sample_freq_matrix[getIndexFromLetter(x), getIndexFromLetter(y)]
	end
end

# ╔═╡ 65c92cac-f930-11ea-20b1-6b8f45b3f262
double_letters = [x for x in alphabet if sample_freq_matrix[getIndexFromLetter(x), getIndexFromLetter(x)] > 0.0]
...
# ╔═╡ 4fd0b42a-8656-11eb-29de-b53c52b7ad61
function findMostProbableFollowingLetter(c)
	mostLikelyToFollow = 1
	maxProbability = 0.0
	allPairs = sample_freq_matrix[getIndexFromLetter(c),:]
	for p ∈ 1:size(allPairs,1)
		if allPairs[p] > maxProbability
			mostLikelyToFollow = alphabet[p]
			maxProbability = allPairs[p]
		end
	end
	return mostLikelyToFollow
end

# ╔═╡ 7898b76a-f930-11ea-2b7e-8126ec2b8ffd
most_likely_to_follow_w = findMostProbableFollowingLetter('w')
...
# ╔═╡ ef01a1c0-8657-11eb-2e07-87937e1b30a2
function findMostProbablePrecedingLetter(c)
	mostLikelyToPrecede = 1
	maxProbability = 0.0
	allPairs = sample_freq_matrix[:, getIndexFromLetter(c)]
	for p ∈ 1:size(allPairs,1)
		if allPairs[p] > maxProbability
			mostLikelyToPrecede = alphabet[p]
			maxProbability = allPairs[p]
		end
	end
	return mostLikelyToPrecede
end

# ╔═╡ bc401bee-f931-11ea-09cc-c5efe2f11194
most_likely_to_precede_w = findMostProbablePrecedingLetter('w')
...
function matrix_distance(A, B)
	distance = abs.(A.-B)
	return sum(distance) # do something with A .- B
end
...
function ngrams(words, n)
	starting_positions = 1:size(words,1)-n+1
	map(i->words[i:i+n-1], starting_positions)
end
...
# ╔═╡ 98acdad4-8651-11eb-2fc9-fb142cf28341
methods(show_pair_frequencies)
